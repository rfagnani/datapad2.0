...
begin
  ...
  if p_include_all and not v_is_partner_admin then
    raise exception 'Not authorized to list all requests';
  end if;
...
create or replace function app.fn_requests_get(
  p_request_id text
)
returns jsonb
language plpgsql
...
begin
  ...
  loop
    ...
    v_sql := format('select to_jsonb(r) from %s r where r.%I::text = %L', v_source, v_id_col, trim(p_request_id));

    if not v_is_partner_admin then
      if v_requester_col is null then
        continue;
      end if;
      v_sql := v_sql || format(' and r.%I::text = %L', v_requester_col, auth.uid()::text);
    end if;
...
grant execute on function app.fn_requests_get(text) to authenticated, service_role;
